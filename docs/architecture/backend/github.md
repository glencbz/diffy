...
export function headChain(
  ...
  if (states.at(-1)?.head !== headRefOid) {
    states.push({ head: headRefOid, origin: { kind: "current" } });
  }

  return states.filter(
    (state, index) => index === 0 || state.head !== states[index - 1]?.head,
  );
}
```
...
describe("headChain", () => {
  test("a PR with no force pushes has one state, where it opened", () => {
    ...
    // assert
    expect(states).toEqual([{ head: oid("a"), origin: { kind: "opened" } }]);
  });
...
describe("githubPullRequestHistory", () => {
  ...
  test("a PR with no force pushes has one state", async () => {
    ...
    expect(history.states).toEqual([
      { head: CHAIN[0] as GitOid, origin: { kind: "opened" } },
    ]);
    expect(history.truncated).toBe(false);
  });
