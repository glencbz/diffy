...
so a non-zero exit almost always means the caller passed a bad one — we
...
that is not a `ShellError` — jj missing from `PATH`, say — is a genuine fault
...
revision against its parent — the "show a commit's diff" operation the
[tech plan](../tech-plan.md) calls for. `--color=never` keeps the output free
of ANSI escapes.
...
metadata the UI needs off each file's header — the change kind and the
affected path(s).
...
`jj --git` names each side's path on its own line: `--- a/…` / `+++ b/…` for
text edits, `rename from` / `rename to` (or `copy …`) for moves, and
`Binary files … differ` for binaries. We read whichever of those is present
and only fall back to the `diff --git a/… b/… ` header for the one case that
has none — a mode-only change. `/dev/null` on a side means that side doesn't
exist. A patch we can't pull a path from is a parser bug, not a jj failure, so
it throws a plain `Error` (a 500) rather than guessing.
...
without touching a repo — the fixtures below are trimmed real `jj diff --git`
output. Only the integration-level behaviour (default revision, error
